...
public class SlimeSwallow : MonoBehaviour
{
    /*──────── 组件引用 ────────*/
    [Header("外部组件")]
    public SlimeAbilityManager abilityMgr;   // EInspector 拖 Slime 进来
    ...
    void Awake()
    {
        ...
        if (!abilityMgr)
            abilityMgr = GetComponent<SlimeAbilityManager>(); // 同蝸E迳献远?
    }
    ...
    void PrintSlots()
    {
        ...
        for (int i = 0; i < slots.Count; i++)
            Debug.Log($"槽{i + 1}: {slots[i].type}{(i == currentIndex ? " [激畸y" : "")}");
    }
