...
public class SlimeController : MonoBehaviour
{
    ...
    public float normalColliderRadius = 0.2f;
    public float crouchColliderRadius = 0.1f;
    ...
    {
        // 严格区分墙跳
        if (isOnWall && Mathf.Abs(wallNormal.x) > 0.9f)
        {
            WallJump();
            return;
        }
    ...
    void EvaluateContacts(Collision2D col)
    {
        foreach (var c in col.contacts)
        {
            ...
            if (!isWallJumping && canStickToWall && Mathf.Abs(n.x) > 0.5f)
            {
                isOnWall = true;
